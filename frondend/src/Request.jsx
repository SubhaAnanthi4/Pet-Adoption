...
const Request = () => {
  ...
  useEffect(() => {
    const fetchPet = async () => {
      try {
        const response = await axios.get(`https://pet-adoption-backend-3rp0.onrender.com/petlist/${id}`);
        ...
        {pet ? (
          <>
            ...
            {pet.image && (
              <img
                className="pet-image"
                src={`https://pet-adoption-backend-3rp0.onrender.com/uploads/${pet.image.replace('E:\\project\\pet-platform\\pet\\backend\\pet-platform\\pet\\public\\uploads\\', '')}`}
                alt={pet.name}
              />
            )}
            {!isRequestSent ? (
